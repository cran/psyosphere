...
des_sum <- function(
  tracks, ctarget, cgaps = "", cname = "sum", drop = TRUE, t_id = "id",
  des_df = ""
) {

  # Check variables
  e <- val_psyo(tracks, 0, 0, 0, 0, 0); if (e != "") {stop(e)}
  # e <- val_cname(tracks, ctarget, type = c("numeric","logical"), def = FALSE); if (e != "") {stop(e)}
  ...
  # Calculate sum for each track
  result <- psyosphere::apply_tracks(
    tracks,
    "sum_private(eval_track, arg1, arg2, arg3, arg4)",
    arg1 = ctarget,
    arg2 = cgaps,
    arg3 = cname,
    arg4 = t_id,
...
sum_private <- function(tracks, ctarget, cgaps, cname, t_id) {

  # Skip gaps
  tracks <- omit_gaps_private(tracks, ctarget, cgaps)

  # Calculate sum
  result <- sum( tracks[,ctarget], na.rm = TRUE)
