...
des_max <- function(
  tracks, ctarget, cgaps = "", cname = "max", drop = TRUE, t_id = "id",
  des_df = ""
) {
  ...
  result <- psyosphere::apply_tracks(
    tracks,
    "max_private(eval_track, arg1, arg2, arg3, arg4)",
    arg1 = ctarget,
    arg2 = cgaps,
    arg3 = cname,
    arg4 = t_id,
...
max_private <- function(tracks, ctarget, cgaps, cname, t_id) {

  # Skip gaps
  tracks <- omit_gaps_private(tracks, ctarget, cgaps)

  # Calculate
  result <- max(tracks[,ctarget], na.rm = TRUE)
