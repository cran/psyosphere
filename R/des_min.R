...
des_min <- function(
  tracks, ctarget, cgaps = "", cname = "min", drop = TRUE, t_id = "id",
  des_df = ""
) {
  ...
  result <- psyosphere::apply_tracks(
    tracks,
    "min_private(eval_track, arg1, arg2, arg3, arg4)",
    arg1 = ctarget,
    arg2 = cgaps,
    arg3 = cname,
    arg4 = t_id,
...
min_private <- function(tracks, ctarget, cgaps, cname, t_id) {

  # Skip gaps
  tracks <- omit_gaps_private(tracks, ctarget, cgaps)

  # Calculate
  result <- min(tracks[,ctarget], na.rm = TRUE)
