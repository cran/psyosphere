...
if (!exists("p_dir")) {
  p_dir <- paste0(demo_dir,"/smuggler2/")
}
...
tracks_guards <- tracks[ tracks$tracker %in% ids_guards ,]
tracks <- tracks[ !tracks$tracker %in% ids_guards ,]
...
plot_tracks(tracks[ tracks[,"ppn"] == 1 ,], plot = plot)
...
tracks_sel <- tracks[ tracks[,"ppn"] == 1 ,]
...
tracks <- plyr::rename(
  tracks, c(
    "dis_to_02.gpx_in_m" = "dis_guard1",
    "dis_to_28.gpx_in_m" = "dis_guard2",
    "dis_to_61.gpx_in_m" = "dis_guard3"
  )
)
...
library(xlsx)
xlsx::write.xlsx(des_summary, paste0(p_dir, "/smuggler2.xlsx"))
...
# 23 illustrate tracks for publication -----------------------------------------

rm(list = setdiff(ls(), "p_dir"))
load(paste0(p_dir, "/rdata/22.RData"))

# select tracks

tracks_sel <- tracks[ tracks[,"team"] == 8 & tracks[,"track"] == 1, ]
tracks_sel[ tracks_sel[,"cocaine"] == 0 , "track_color"] <- "#FFFF00FF"
tracks_sel[ tracks_sel[,"cocaine"] == 1 , "track_color"] <- "#000000FF"
...
plot <- plot_polygon(poly_finish, plot = plot)
plot <- plot_polygon(poly_guard, plot = plot)
plot_tracks(tracks_sel, plot = plot, dots = FALSE)

rm(plot, tracks_sel)

save(list = setdiff(ls(), "p_dir"), file = paste0(p_dir, "/rdata/23.RData"))
