...
url <- "https://analyse-gps.com/downloads/smuggle2.zip"
...
guard_ids <- c(2,61,28)
tracks_guards <- tracks[ tracks$tracker %in% guard_ids ,]
tracks <- tracks[ !tracks$tracker %in% guard_ids ,]
...
tracks[,"hist_track_id"] <- paste0("S2_",tracks[,"id"])
...
library(xlsx)
xlsx::write.xlsx(des_summary, paste0(dir, "/smuggler2.xlsx"))
...
# 22 illustrate tracks for publication -----------------------------------------

rm(list = setdiff(ls(), "dir"))
load(paste0(dir, "/rdata/21.RData"))

# select tracks

tracks_sel <- tracks[ tracks[,"team"] == 15 & tracks[,"track"] == 1, ]
...
save.image(paste0(dir, "/rdata/22.RData"))
