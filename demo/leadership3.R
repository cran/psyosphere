...
if (!exists("p_dir")) {
  p_dir <- paste0(demo_dir,"/leadership3/")
}
...
url <- "https://analyse-gps.com/downloads/leadership3.zip"
...
plot_tracks(
  tracks, single = FALSE, save_dir = paste0(p_dir, "/plots/all")
)
...
summary <- des_first(tracks, "comment", cname = "comment", des_df = summary)
...
library(xlsx)
xlsx::write.xlsx(summary, paste0(p_dir, "/leadership3.xlsx"))
